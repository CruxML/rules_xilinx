...
def _vitis_generate_impl(ctx):
    output_file = ctx.actions.declare_file("run_hls_{}.tcl".format(ctx.label.name))
...
def _vivado_bitstream_impl(ctx):
    ...
    run_tcl = ctx.actions.declare_file("run_bitstream_{}.tcl".format(ctx.label.name))
    ...
    ctx.actions.run(
        ...
        inputs = [],
        arguments = args,
        progress_message = "Generating run  _bitstream_{}.tcl".format(ctx.label.name),
...
def _xsim_test_impl(ctx):
    run_tcl = ctx.actions.declare_file("run_xsim_{}.tcl".format(ctx.label.name))
    vivado_log = ctx.actions.declare_file("{}.log".format(ctx.label.name))
